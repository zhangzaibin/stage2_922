...
mcnet = MCNet(11,1.5).to(my_device)
feature_ex = mcnet.feature_extraction
# print(feature_ex)
# checkpoint = torch.load("./stage1_pretrained/dpsnet_2_checkpoint.pth.tar")['state_dict']
checkpoint = torch.load("./stage1_pretrained/new_gaussion+caattention/dpsnet_3_checkpoint.pth.tar")['state_dict']
...
########
    def generate_features_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            # gennerate the concat feature to use channel attention method
            fea_ca = torch.cat((feature_ex.encoder(inputs[("color", 1, source_scale)])[-1], feature_ex.encoder(inputs[("color", 0, source_scale)])[-1]), dim=1)
            fea_ca = torch.cat((fea_ca, feature_ex.encoder(inputs[("color", 0, source_scale)])[-1]), dim=1)
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                outputs[('channel_weights')] = channel_weights
                outputs[("color", frame_id, scale)] = F.grid_sample(
                    feature_ex(inputs[("color", frame_id, source_scale)])[("f_disp", scale)]*channel_weights,
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            target = feature_ex(inputs[("color", 0, source_scale)])[("f_disp", scale)]*outputs[('channel_weights')]
            ...
            if not self.opt.disable_automasking:
                ...
                for frame_id in self.opt.frame_ids[1:]:
                    pred = F.interpolate(feature_ex(inputs[("color", frame_id, source_scale)])[("f_disp", scale)]*outputs[('channel_weights')], [48, 160], mode='bilinear')
            ...
            # loss += 0.02 * torch.sum(feature_losses) / (torch.sum(fea_mask[:, :, 5:43, 5:155]))
            loss += 0.3 * torch.sum(feature_losses) / (torch.sum(fea_mask))
            #loss += 0.2*feature_losses.mean()
            feature_loss_total += 0.3 * torch.sum(feature_losses) / (torch.sum(fea_mask))
                ...
                for frame_id in self.opt.frame_ids:
                    ...
                    if s == 0 and frame_id != 0:
                        ...
                        writer.add_image(
                            ...
                            fea_pred, self.step)
                        fea_ori = colormap((feature_ex(inputs[("color", frame_id, s)])[("f_disp", 0)]*outputs[('channel_weights')])[j].data.mean(0, True))
                        #print(feature_ex(inputs[("color", frame_id, s)])[("f_disp", 0)]*outputs[('channel_weights')][j].data.mean(0, True).shape)
                        writer.add_image(
                        ...
                        writer.add_image(
                            ...
                            outputs["fea_loss_visual/{}".format(s)][j].data*10.0, self.step)
                        writer.add_image(
